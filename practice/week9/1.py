...
class Coin(pygame.sprite.Sprite):
  ...
  def __init__(self):
    super().__init__()
    self.image = pygame.image.load("Racer\\free-icon-dollar-coin-9787486.png")
    self.rect = self.image.get_rect()
    self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)
...
class BigCoin(pygame.sprite.Sprite):
  def __init__(self):
    super().__init__()
    self.image = pygame.image.load('Racer\\free-icon-dollar-coin-9787486-fotor-2024030511740.png')
    self.rect = self.image.get_rect()
  def move(self):
    self.rect.move_ip(0, SPEED)
